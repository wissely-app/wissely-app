...
async function processPaddleEvent(eventType, data, env, request, requestId) {
  ...
  switch (eventType) {
    ...
    case 'subscription.created': {
      ...
      if (userId) {
        await env.DB.prepare(
          ...
          'paddle_subscription_id = ?, subscription_status = ?, scheduled_downgrade_at = NULL WHERE id = ?'
        ...
        await env.DB.prepare(
          ...
          'subscription_status = ?, scheduled_downgrade_at = NULL WHERE paddle_customer_id = ?'
    ...
    // Quota resets happen in the monthly cron, not here. If Paddle has a
    // pending cancellation scheduled (scheduled_change.action === 'cancel'),
    // that effective date is recorded but the plan/quota are left alone —
    // the account keeps full access until the scheduled date actually
    // arrives. Any other scheduled_change (e.g. the cancellation being
    // reversed) clears the pending downgrade.
    case 'subscription.updated': {
      ...
      if (userId) {
        await env.DB.prepare(
          'UPDATE users SET plan = ?, analyses_limit = ?, paddle_customer_id = ?, ' +
          'paddle_subscription_id = ?, subscription_status = ?, scheduled_downgrade_at = ? WHERE id = ?'
        ).bind(plan, analyses_limit, customerId, subscriptionId, status, scheduledDowngradeAt, userId).run();
      } else if (subscriptionId) {
        await env.DB.prepare(
          'UPDATE users SET plan = ?, analyses_limit = ?, subscription_status = ?, scheduled_downgrade_at = ? ' +
          'WHERE paddle_subscription_id = ?'
        ).bind(plan, analyses_limit, status, scheduledDowngradeAt, subscriptionId).run();
      } else {
        console.warn('[Paddle] subscription.updated — no resolvable user, skipped');
      }
      await log('paddle_subscription_updated', { plan, status, subscriptionId, scheduledDowngradeAt });
      break;
    }
    ...
    // confirmation of active status after every real payment. A completed
    // payment always means the subscription is in good standing going
    // forward, so any pending scheduled downgrade is cleared here too.
...
export default {
  ...
  //   "0 0 1 * *"   — monthly, 1st of month: usage reset + full cleanup.
  ...
  async scheduled(event, env, ctx) {
    ...
    // ── Monthly job — usage reset + full cleanup ──────────────────────────────
    if (cronExpr === '0 0 1 * *') {
...
//
// 2. users table — add the scheduled-downgrade column used by the
//    period-end-aware cancellation flow. NULL means "no pending downgrade".
//    ALTER TABLE users ADD COLUMN scheduled_downgrade_at TEXT;
//
// 3. Audit log table:
...
//    trigger performs usage reset and full data cleanup. See scheduled()
//    for how event.cron is used to route between them.
//
