...
// One-time "pay only for what you use" prices - the new checkout model.
// Nothing is a subscription anymore: buying a pack tops up analyses_limit
// (additive - previously-purchased, unused quota is never lost) and sets
// plan to whichever pack was most recently purchased (most-recent-purchase
// wins for feature gating, e.g. Pro-only API access/team seats/webhooks).
// analyses_used is never touched by a purchase.
//
// Deliberately kept separate from PADDLE_PRICE_PLANS above rather than
// merged into it - the two have different quota semantics (reset-to-fresh
// vs additive) and are driven by different Paddle events (subscription.*
// vs a subscription-less transaction.completed), so keeping them as
// distinct maps keeps that logic simple to reason about at each call site.
const PADDLE_ONE_TIME_PRICE_PLANS = {
  'pri_01m0qgvm8nffm6fc697vzce00f': { plan: 'starter', analyses_limit: 50   },
  'pri_01m0qgs8g6x01h4qn6d8yswh1e': { plan: 'growth',  analyses_limit: 250  },
  'pri_01m0qgm4qyp89enmg1tw5447g5': { plan: 'pro',     analyses_limit: 1000 },
};

const PLAN_TO_ONE_TIME_PRICE_ID = Object.entries(PADDLE_ONE_TIME_PRICE_PLANS).reduce((acc, [priceId, cfg]) => {
...
async function parseJsonBody(request, maxBytes = MAX_REQUEST_BODY_BYTES) {
  ...
  if (contentLength && !isNaN(Number(contentLength)) && Number(contentLength) > maxBytes) {
    return { body: null, error: 'Request body too large', tooLarge: true };
  }
...
// That single SQL statement is what actually prevents quota bypass; this lock
// exists to avoid wasting an Anthropic API call (and money) on a request that
// is virtually certain to be rejected, and to give the user a clear, fast
...
async function acquireAnalysisLock(env, userId) {
  ...
  try {
    ...
    await env.RATE_LIMIT_KV.put(key, '1', { expirationTtl: ANALYZE_LOCK_TTL_SECONDS });
    return true;
  } catch {
...
// Returns the number of seconds remaining on an active login block, or null
// if not blocked. Previously this stored the literal string 'blocked' and
// relied on KV's own expirationTtl to clear it - correct for blocking, but
// gave the caller no way to tell the user how long is actually left. Now
// stores the block's real expiry timestamp as the value so that can be
// calculated directly, without a second KV call or read-time metadata.
async function checkLoginBlock(request, env, email) {
  ...
  try {
    ...
    const expiries = [ipBlock, emailBlock]
      .map(v => v ? parseInt(v, 10) : NaN)
      .filter(n => !isNaN(n));
    if (!expiries.length) return null;

    const remainingSeconds = Math.ceil((Math.max(...expiries) - Date.now()) / 1000);
...
async function recordLoginFailure(request, env, email) {
  ...
  try {
    ...
    const blockExpiresAt = String(Date.now() + LOGIN_BLOCK_TTL_SECONDS * 1000);
    if (ipCount    >= LOGIN_BLOCK_THRESHOLD) writes.push(env.RATE_LIMIT_KV.put(`login_block:ip:${ip}`,       blockExpiresAt, { expirationTtl: LOGIN_BLOCK_TTL_SECONDS }));
    if (emailCount >= LOGIN_BLOCK_THRESHOLD) writes.push(env.RATE_LIMIT_KV.put(`login_block:email:${email}`, blockExpiresAt, { expirationTtl: LOGIN_BLOCK_TTL_SECONDS }));
...
function validateMessagesPayload(messages) {
  ...
  if (messages.length > MAX_MESSAGES_COUNT) {
    return `messages cannot contain more than ${MAX_MESSAGES_COUNT} items`;
  }
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    if (typeof m.content === 'string') {
      if (m.content.trim().length === 0) return `messages[${i}].content must not be empty`;
      ...
      for (const block of m.content) {
        if (!block || typeof block !== 'object' || typeof block.type !== 'string') {
          return `messages[${i}].content contains an invalid block`;
        }
      }
    } else {
...
// Validates the optional body.tool field. Unknown-but-well-formed tool names
// are allowed through (buildSystemPrompt() falls back to the base prompt),
// but the field is still bounded so it can never be used to smuggle an
// oversized or malformed value into logs or downstream prompts.
function validateToolField(tool) {
  if (tool === undefined || tool === null) return null;
  if (typeof tool !== 'string' || tool.length === 0 || tool.length > 64) {
    return 'tool must be a non-empty string of 64 characters or fewer';
  }
  if (!/^[a-zA-Z0-9_-]+$/.test(tool)) {
    return 'tool contains invalid characters';
  }
  return null;
}
...
// Beyond the top-level schema, each array item is also normalised field-by-
// field so the frontend never receives unexpected nested types (objects,
// arrays, etc.) where a string was expected.

function toSafeString(value, fallback = '') {
  if (typeof value === 'string') return value;
  if (value === null || value === undefined) return fallback;
  if (typeof value === 'object') return fallback; // never stringify nested objects into the UI
  return String(value);
}

function toSafeLevel(value) {
  return ['low', 'medium', 'high'].includes(value) ? value : 'medium';
}

function sanitizeAIArray(arr, itemSanitizer) {
...
function validateAIReport(raw) {
  ...
  return {
    ...
    metrics: sanitizeAIArray(report.metrics, item => ({
      label: toSafeString(item.label),
      value: toSafeString(item.value),
      unit:  toSafeString(item.unit)
    })),

    findings: sanitizeAIArray(report.findings, item => ({
      title:  toSafeString(item.title),
      detail: toSafeString(item.detail)
    })),

    risks: sanitizeAIArray(report.risks, item => ({
      level:       toSafeLevel(item.level),
      description: toSafeString(item.description)
    })),

    recommendations: sanitizeAIArray(report.recommendations, item => ({
      priority: toSafeLevel(item.priority),
...
// practice Paddle's own delivery/backoff behavior makes this exceedingly
// unlikely, and every DB write inside processPaddleEvent() is itself an
// idempotent UPDATE keyed by subscription/user ID, so a rare double-run is
// harmless (it just re-applies the same state). For a mathematically exact
// guarantee, back this with a Durable Object instead of KV.

async function getPaddleEventState(eventId, env) {
  if (!env.RATE_LIMIT_KV || !eventId) return null;
  try {
    return await env.RATE_LIMIT_KV.get(`${PADDLE_EVENT_KV_PREFIX}${eventId}`);
  } catch {
    return null;
  }
}

async function setPaddleEventState(eventId, env, state, ttlSeconds) {
  if (!env.RATE_LIMIT_KV || !eventId) return;
  try {
    await env.RATE_LIMIT_KV.put(`${PADDLE_EVENT_KV_PREFIX}${eventId}`, state, { expirationTtl: ttlSeconds });
  } catch {
    console.warn('[Paddle] Failed to write event state to KV:', eventId, state);
  }
}

async function deletePaddleEventState(eventId, env) {
  if (!env.RATE_LIMIT_KV || !eventId) return;
  try {
...
// Same shape as resolvePaddlePlan, checked against the one-time price map
// instead - used for genuine one-time purchases (no subscription_id on the
// transaction at all), see the transaction.completed handler below.
function resolveOneTimePlan(items) {
  if (!Array.isArray(items)) return null;
  for (const item of items) {
    const cfg = PADDLE_ONE_TIME_PRICE_PLANS[item?.price?.id];
    if (cfg) return cfg;
  }
  return null;
}
...
// passed. This avoids cutting off a customer's access mid-billing-period
  ...
  switch (eventType) {
    ...
    case 'subscription.created': {
      ...
      if (userId) {
        await env.DB.prepare(
          ...
          'paddle_subscription_id = ?, subscription_status = ?, scheduled_downgrade_at = NULL, ' +
          'analyses_used = 0, current_period_starts_at = ? WHERE id = ?'
        ).bind(plan, analyses_limit, customerId, subscriptionId, status, periodStart, userId).run();
      } else if (customerId) {
        await env.DB.prepare(
          'UPDATE users SET plan = ?, analyses_limit = ?, paddle_subscription_id = ?, ' +
          'subscription_status = ?, scheduled_downgrade_at = NULL, ' +
    ...
    case 'subscription.activated': {
      ...
      await env.DB.prepare(
        ...
        'analyses_used = 0, current_period_starts_at = ? ' +
        'WHERE paddle_subscription_id = ?'
      ).bind(plan, analyses_limit, status, periodStart, subscriptionId).run();
    ...
    case 'subscription.updated': {
      ...
      if (userId) {
        await env.DB.prepare(
          'UPDATE users SET plan = ?, analyses_limit = ?, paddle_customer_id = ?, ' +
          'paddle_subscription_id = ?, subscription_status = ?, scheduled_downgrade_at = ?, ' +
          'analyses_used = CASE WHEN ? IS NOT NULL AND (current_period_starts_at IS NULL OR current_period_starts_at != ?) ' +
          'THEN 0 ELSE analyses_used END, ' +
          'current_period_starts_at = COALESCE(?, current_period_starts_at) ' +
          'WHERE id = ?'
        ).bind(
          plan, analyses_limit, customerId, subscriptionId, status, scheduledDowngradeAt,
          periodStart, periodStart, periodStart, userId
        ).run();
      } else if (subscriptionId) {
        await env.DB.prepare(
          'UPDATE users SET plan = ?, analyses_limit = ?, subscription_status = ?, scheduled_downgrade_at = ?, ' +
          'analyses_used = CASE WHEN ? IS NOT NULL AND (current_period_starts_at IS NULL OR current_period_starts_at != ?) ' +
          'THEN 0 ELSE analyses_used END, ' +
          'current_period_starts_at = COALESCE(?, current_period_starts_at) ' +
          'WHERE paddle_subscription_id = ?'
        ).bind(
          plan, analyses_limit, status, scheduledDowngradeAt,
    ...
    // ── Cancellation ─────────────────────────────────────────────────────────
    // Fires when the subscription's status actually transitions to
    // 'canceled'. If Paddle indicates a future effective date (either via
    // scheduled_change or the current billing period's end), the plan is
    // preserved and the downgrade is deferred to that date via
    // scheduled_downgrade_at. Only when there is no future date (immediate
    // cancellation, e.g. via API with effective_from: immediately) is the
    // account downgraded to Free right away.
    case 'subscription.canceled': {
      const effectiveAt =
        ...
        null;
      const now = Date.now();
      const hasFutureEffectiveDate = effectiveAt && new Date(effectiveAt).getTime() > now;

      if (hasFutureEffectiveDate) {
        await env.DB.prepare(
          'UPDATE users SET subscription_status = ?, scheduled_downgrade_at = ? WHERE paddle_subscription_id = ?'
        ).bind('canceled', effectiveAt, subscriptionId).run();
    ...
    case 'subscription.resumed': {
      ...
      await env.DB.prepare(
        ...
        'current_period_starts_at = COALESCE(?, current_period_starts_at) ' +
        'WHERE paddle_subscription_id = ?'
      ).bind(plan, analyses_limit, status, periodStart, periodStart, periodStart, subscriptionId).run();
    ...
    // subscription.updated was delayed or dropped. A completed payment
    // always means the subscription is in good standing going forward, so
    // any pending scheduled downgrade is cleared here too. Paddle transaction
    // objects carry their own `billing_period` for subscription charges,
    // used the same way current_billing_period is used elsewhere.
    case 'transaction.completed': {
      const txnSubId      = data?.subscription_id;
      const txnPlanConf   = resolvePaddlePlan(data?.items);
      const txnPeriodStart = data?.billing_period?.starts_at ?? null;
      if (txnSubId && txnPlanConf) {
        const { plan, analyses_limit } = txnPlanConf;
        await env.DB.prepare(
          "UPDATE users SET plan = ?, analyses_limit = ?, subscription_status = 'active', scheduled_downgrade_at = NULL, " +
          'analyses_used = CASE WHEN ? IS NOT NULL AND (current_period_starts_at IS NULL OR current_period_starts_at != ?) ' +
          'THEN 0 ELSE analyses_used END, ' +
          'current_period_starts_at = COALESCE(?, current_period_starts_at) ' +
          'WHERE paddle_subscription_id = ?'
        ).bind(plan, analyses_limit, txnPeriodStart, txnPeriodStart, txnPeriodStart, txnSubId).run();
        await log('paddle_transaction_completed', { plan, subscriptionId: txnSubId, periodStart: txnPeriodStart });
        break;
      }

      // No subscription_id - a genuine one-time "pay only for what you use"
      // purchase, not a recurring subscription's periodic charge. There's
      // no subscription row to match the user against here, so identity
      // relies on the userId already resolved above via
      // resolveWebhookUserId - custom_data.user_id first (set at checkout
      // creation, see createPaddleCheckoutTransaction), falling back to a
      // Paddle customer-email lookup if that's ever missing.
      //
      // Quota is additive, not reset: analyses_limit increases by the
      // purchased pack's size and analyses_used is never touched, so
      // previously-purchased, unused quota is never lost. plan is set to
      // whichever pack was just bought - most-recent-purchase wins for
      // which plan's features (API access, team seats, webhooks) apply,
      // even if the customer is drawing down a mixed-size quota balance.
      const oneTimePlanConf = resolveOneTimePlan(data?.items);
      if (oneTimePlanConf && userId) {
        const { plan, analyses_limit } = oneTimePlanConf;
        const customerId = data?.customer_id ?? null;
        const result = await env.DB.prepare(
          'UPDATE users SET plan = ?, analyses_limit = analyses_limit + ?, ' +
          'paddle_customer_id = COALESCE(paddle_customer_id, ?) WHERE id = ?'
        ).bind(plan, analyses_limit, customerId, userId).run();

        if (result.meta.changes > 0) {
          await log('paddle_one_time_purchase_completed', { plan, analysesAdded: analyses_limit });
        } else {
          console.warn('[Paddle] One-time purchase completed but user_id not found:', userId);
        }
      }
      break;
    }
...
// Downgrades any account whose scheduled cancellation date has passed.
// Intended to run daily (see scheduled() below) so a customer who canceled
// mid-cycle keeps access for the remainder of the period they already paid
// for, and loses it promptly (within a day) once that period actually ends.
async function downgradeExpiredCancellations(env) {
  const now = new Date().toISOString();
  try {
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (path === '/webhook/paddle' && request.method === 'POST') {
        ...
        // Reject oversized webhook payloads before buffering the full body.
        const declaredLength = request.headers.get('Content-Length');
        if (declaredLength && !isNaN(Number(declaredLength)) && Number(declaredLength) > MAX_WEBHOOK_BODY_BYTES) {
        ...
        if (eventState === 'processing') {
          ...
          return createResponse(request, { received: true, retryLater: true }, 202);
        }
        ...
        try {
          ...
          await deletePaddleEventState(eventId, env);
      ...
      if (path === '/login' && request.method === 'POST') {
        ...
        const blockedForSeconds = await checkLoginBlock(request, env, email);
        if (blockedForSeconds) {
          ...
          return createResponse(request, {
            ...
            retryAfterSeconds: blockedForSeconds
          }, 429, { 'Retry-After': String(blockedForSeconds) });
        }
      ...
      if (path === '/me' && request.method === 'GET') {
        ...
        const flags = computeAccessFlags(session);
      ...
      // point - the price ID is resolved server-side from
      // PLAN_TO_ONE_TIME_PRICE_ID, which is derived from
      // PADDLE_ONE_TIME_PRICE_PLANS. Every checkout created here is a
      // one-time "pay only for what you use" purchase, not a subscription -
      // see PADDLE_ONE_TIME_PRICE_PLANS above for why quota is additive.
      ...
      if (path === '/create-checkout' && request.method === 'POST') {
        ...
        const { plan } = body ?? {};
        const priceId   = typeof plan === 'string' ? PLAN_TO_ONE_TIME_PRICE_ID[plan] : undefined;
        ...
        // No "already on this plan" block here, deliberately - under the
        // pay-only-for-what-you-use model, buying the same plan again is a
        // normal refill purchase, not a redundant duplicate. Quota stacks
        // (see the transaction.completed handler), so there's nothing to
        // protect the user from by blocking this.
      ...
      if (path === '/analyze' && request.method === 'POST') {
        ...
        const preFlightFlags = computeAccessFlags(session);
        if (preFlightFlags.upgradeRequired) {
          return createResponse(request, {
            upgradeRequired: true,
            trialExpired:    preFlightFlags.trialExpired,
      ...
      // customer checking on an incident reads. Lives at /api/status, not
      // the bare /status, deliberately - Cloudflare's static asset serving
      // for status.html can intercept a request to /status before this
      // Worker route ever runs, which silently returns the HTML page
      // instead of JSON and breaks status.html's own fetch() call to
      // itself. Backed by the same RATE_LIMIT_KV binding as everything
      // else (no new KV namespace needed to ship this) under a dedicated
      // key prefix. There is no admin UI in v1 - status is updated by
      // editing the KV value directly from the Cloudflare dashboard.
      // Defaults to "operational" with no incidents if the key has never
      // been set, so the page never breaks before anyone's configured it.
  ...
  // A trial account's 20-analysis allowance is a one-time allocation and is
  // never reset by a cron job; it only changes when the account upgrades.
...
//
//    current_period_starts_at tracks the start of the user's current
...
//    CREATE INDEX IF NOT EXISTS idx_users_scheduled_downgrade  ON users(scheduled_downgrade_at);
